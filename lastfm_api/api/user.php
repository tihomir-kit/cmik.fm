...
class lastfmApiUser extends lastfmApi {
	...
	public function getInfo($methodVars = Array()) {
		...
		if ( !empty($methodVars['user']) ) {
			...
			$this->handleError(92, 'No such user');
			return FALSE;
		}
	}
...
//						$topartists[$i]['images']['medium'] = (string) $artist->image[1];
						$topartists[$i]['image'] = (string) $artist->image[2];
						$i++;
					}
	 ...
	 * @return array
	 */
	public function getArtistTracks($methodVars) {
		// Check for required variables
		if ( !empty($methodVars['user']) ) {
			$vars = array(
			...
			if ( $call = $this->apiGetCall($vars) ) {
				$i = 0;
				foreach ( $call->artisttracks->track as $track ) {
					$artisttrack[$i]['name'] = (string) $track->name;
					$i++;
				}
